...
class VerifiableExtension extends DataExtension
{
    ...
    public function onAfterPublish()
    {
        ...
        // source() which makes use of this data
        DB::query(sprintf(''
            . ' UPDATE "%s"'
    ...
    public function source($record = null) : array
    {
        ...
        if (method_exists($record, 'verify')) {
            ...
            // Ensure the DB field: "VerifiableFields" is in the list of fields who's content is hashed
            // If we don't, attacking that field would be a source of verification errors,
            // where a developer could change the `verifiable_fields` YML config, thus affecting
            // _all_ versions.
            // With the following logic however; the list of fields to be verified is
            // relative to the current version.
            $cachedFieldList = $this->getOwner()->dbObject('VerifiableFields')->getStoreAsArray();
            $verifiableFieldList = array_merge(['VerifiableFields'], $cachedFieldList);

            // Our list of verifiable fields comes from the "VerifiableFields" DB field (if not empty).
            // Otherwise, we default to using the list out of the YML config `verifiable_fields`.
            $fields = ($cachedFieldList && $this->getOwner()->getField('VerifiableFields')) ?
                    $verifiableFieldList :
                    $record->config()->get('verifiable_fields');
